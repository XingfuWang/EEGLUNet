...
class signal2spd(nn.Module):
    ...
    def forward(self, x):
        ...
        tra = tra.view(-1, 1, 1)
        cov /= tra
        identity = torch.eye(cov.shape[-1], cov.shape[-1], device=x.device).to(x.device).repeat(x.shape[0], 1, 1)
...
class AttentionManifold(nn.Module):
    ...
    def forward(self, x, shape=None):
        ...
        Q = self.q_trans(x).view(bs, m, self.d_out, self.d_out).to(x.device)
        K = self.k_trans(x).view(bs, m, self.d_out, self.d_out).to(x.device)
        V = self.v_trans(x).view(bs, m, self.d_out, self.d_out).to(x.device)
...
class mAtt(nn.Module):
    def __init__(self, epochs, channel=22, n_outputs=4):
        super().__init__()
        # FE
        # bs, 1, channel, sample
        ...
        # bs, 22, 1, sample
        self.conv2 = nn.Conv2d(channel, 20, (1, 12), padding=(0, 6))
        ...
        # fc
        self.linear = nn.Linear(9 * 19 * epochs, n_outputs, bias=True)
...
if __name__ == '__main__':
    x = torch.randn(20, 22, 1000).to('cuda')
    model = mAtt(epochs=10, channel=22, n_outputs=4).to('cuda')
    y = model(x)
