...
class EEGLUNet_abl_LU(nn.Module):

    def __init__(self, n_chans: int, n_class: int = 4, spatial_expansion: int = 32, spatial_merge: int = 32, filters: list = None):  # 'gct','catlite'
        super().__init__()
...
if __name__ == '__main__':
    ...
    model = EEGLUNet_abl_LU(22, 4).to('cpu').float()
    y = model(x)
    print(y.shape)
